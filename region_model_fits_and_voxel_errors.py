...
def cross_val_errors(X,Y,P_X,P_Y,P_Y_dag,err_fun):
    ...
    for i,(train,test) in enumerate(outer_sets):
        ...
        rel_err_reg[i]=err_reg[i]/(err_fun(Y_test_reg)+err_fun(Y_pred))
        rel_err_homog[i]=err_homog[i]/(err_fun(Y_test)+err_fun(Y_pred_homog))
        ...
        GOF_homog[i]=err_fun(P_Y_dag.dot(W.dot(P_X.dot(X_train)))-Y_train)/\
          (err_fun(Y_train) + err_fun(W.dot(P_X.dot(X_train))))
...
errs_ipsi=cross_val_errors(X,Y_ipsi,P_X,P_Y_ipsi,P_Y_ipsi_dag,
                           error_MSE)
errs_contra=cross_val_errors(X,Y_contra,P_X,P_Y_contra,P_Y_contra_dag,
                             error_MSE)

errs = pd.DataFrame(np.vstack((errs_ipsi,errs_contra)).T,
                    columns=['err_reg_ipsi','err_homog_ipsi',
                             'rel_err_reg_ipsi','rel_err_homog_ipsi',
                             'err_train_ipsi','err_train_homog_ipsi',
                             'err_reg_contra', 'err_homog_contra',
                             'rel_err_reg_contra','rel_err_homog_contra',
